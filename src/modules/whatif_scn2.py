...
def fte_impact_of_demand_increase(df, model, q2, abn, occ, occ_assmp, weighted_avg_staff):
    ...
    for change in demand_changes:
        ...
        fte_pred = model.predict(pd.DataFrame({
            "Q2": [q2], "ABN %": [abn], "Occ Assumption": [occ_assmp],
            "Staffing": [weighted_avg_staff], "Demand": [demand_scenario], "Occupancy Rate": [occ]
        }))[0]
        predictions.append((change, demand_scenario, fte_pred))
    return predictions

def impact_of_occ_assumption_change(df, model, q2, abn, occ, occ_assmp, weighted_avg_demand, weighted_avg_staff):
...
def scn2():
    ...
    if uploaded_file is not None:
        ...
        fte_prediction = analyze_scenarios(weekly_df, model, q2, abn/100, occ/100, occ_assmp/100)
        ...
        demand_impact = fte_impact_of_demand_increase(weekly_df, model, q2, abn/100, occ/100, occ_assmp/100, weighted_avg_staff)
        for change, demand_scenario, fte_pred in demand_impact:
            st.write(f"Demand Increase = {change}% | New Weekly Demand = {demand_scenario * 7:.2f}: | Predicted FTE Requirement= {fte_pred:.2f}")
        ...
        occ_impact = impact_of_occ_assumption_change(weekly_df, model, q2, abn/100, occ/100,occ_assmp/100, weighted_avg_demand, weighted_avg_staff)
